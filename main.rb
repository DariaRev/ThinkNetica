...
class RailRoad
  ...
  def seed
    ...
    @trains = [Train.new(12_345, CargoType.new, 'g'), Train.new(23_456, CargoType.new, 'h'),
               Train.new(34_567, PassType.new, 'h')]
  ...
  def info(show_trains = FALSE)
    ...
    (1..@stations.length).each do |i|
      ...
      station.trains_each { |x| puts x.info }
    end
  end
  ...
  def info_trains
    ...
    ans = gets.chomp.to_i
    station = @stations[ans - 1]
    station.trains_each { |x| puts x.info }
  end

  def info_cars
    train = select_train_by_num
    train.cars_each { |x| puts x.info }
  end

  def info_smth
    puts '1 - вывести информацию обо всех станциях'
    puts '2 - вывести информацию обо всех поездах на станции'
    puts '3 - вывести информацию обо всех вагонах поезда'
    puts '0 - exit'
    ans = gets.chomp.to_i
    info if ans == 1
    info_trains if ans == 2
    info_cars if ans == 3
  end
  ...
  def add_car
    ...
    capacity = gets.chomp
    car = Car.new(found.type, capacity)
    found.add_car(car)
  end
  ...
  def edit_car_capacity
    ...
    if train.type.instance_of? CargoType
      puts 'Введите объем, который будет занимать груз'
      ans = gets.chomp.to_f
      car.add(ans)
    else
      car.add
